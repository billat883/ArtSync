...
export class FhevmReactError extends Error { 
  code: string; 
  constructor(code: string, message?: string, options?: ErrorOptionsCompat) { 
...
async function getWeb3Client(rpcUrl: string) {
  ...
  try { const version = await rpc.send("web3_clientVersion", []); return version; }
  catch (e) { throwFhevmError("WEB3_CLIENTVERSION_ERROR", `The URL ${rpcUrl} is not a Web3 node or unreachable.`, e); }
  finally { rpc.destroy(); }
}
...
async function getFHEVMRelayerMetadata(rpcUrl: string) {
  ...
  catch (e) { throwFhevmError("FHEVM_RELAYER_METADATA_ERROR", `The URL ${rpcUrl} is not a FHEVM Hardhat node or unreachable.`, e); }
  finally { rpc.destroy(); }
}
...
export const createFhevmInstance = async (parameters: {
  ...
  const aclAddress = (relayerSDK.SepoliaConfig as any).aclContractAddress;
  ...
  const config: FhevmInstanceConfig = { ...(relayerSDK.SepoliaConfig), network: providerOrUrl, publicKey: pub.publicKey, publicParams: pub.publicParams } as any;
  notify("creating");
