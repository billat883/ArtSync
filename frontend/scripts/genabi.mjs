...
const OUT_ABI = path.resolve(process.cwd(), "src", "abi", "ArtExpoABI.ts");
const OUT_ADDR = path.resolve(process.cwd(), "src", "abi", "ArtExpoAddresses.ts");
...
for (const net of networks) {
  ...
  const em = loadDeployment(dir, "ArtExpoFHE");
  if (em) {
...
if (abi) {
  ...
  fs.writeFileSync(OUT_ABI, `export const ArtExpoABI = {\n  abi: ${JSON.stringify(sanitized, null, 2)}\n};\n`);
}
fs.writeFileSync(OUT_ADDR, `export const ArtExpoAddresses = ${JSON.stringify(addresses, null, 2)} as const;\n`);
